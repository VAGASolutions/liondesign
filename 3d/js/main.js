...
/* ══════════════════════════════════════════
   ⚡ LIGHTNING STRIKE SCROLL TRANSITIONS
══════════════════════════════════════════ */
(function initLightning() {
  ...
  // Returns array of {x1,y1,x2,y2,width,alpha} segments
  function generateBolt(x1, y1, x2, y2, depth, branchDecay) {
    if (depth === 0) return [{ x1, y1, x2, y2 }];

    const mx  = (x1 + x2) / 2 + (Math.random() - 0.5) * (Math.abs(y2 - y1) * 0.75);
    const my  = (y1 + y2) / 2 + (Math.random() - 0.5) * (Math.abs(y2 - y1) * 0.15);
    const segs = [
      ...generateBolt(x1, y1, mx, my, depth - 1, branchDecay),
      ...generateBolt(mx, my, x2, y2, depth - 1, branchDecay),
    ];

    // random sub-branch
    if (depth >= 2 && Math.random() < 0.55) {
      const angle  = (Math.random() - 0.5) * Math.PI * 0.9;
      const length = (Math.abs(y2 - y1) * 0.35 + Math.abs(x2 - x1) * 0.35) * branchDecay;
      const bx2 = mx + Math.sin(angle) * length;
      const by2 = my + Math.cos(angle) * length;
      segs.push(...generateBolt(mx, my, bx2, by2, depth - 2, branchDecay * 0.6));
    }
    return segs;
  }

  /* ── Draw one bolt set with multi-pass glow ──────── */
  function drawBolt(segs, life) { // life 0→1, 1=new 0=dead
    const alpha = life;

    // pass 1 — wide amber corona
    ctx.save();
    ...
    segs.forEach(s => { ctx.beginPath(); ctx.moveTo(s.x1, s.y1); ctx.lineTo(s.x2, s.y2); ctx.stroke(); });
    ctx.restore();

    // pass 2 — mid amber glow
    ctx.save();
    ...
    // pass 4 — pure white core (thinnest)
    ctx.save();
  ...
  class Spark {
    ...
    draw() {
      ...
      ctx.fillStyle   = `hsl(${40 + this.life * 20}, 100%, ${60 + this.life * 30}%)`;
      ctx.shadowColor = '#f59e0b';
      ctx.shadowBlur  = 6;
      ctx.beginPath();
  ...
  /* ── Active bolts & sparks ───────────────────────── */
  let activeBolts = []; // { segs, life, decay }
  let sparks      = [];

  /* ── Strike! ─────────────────────────────────────── */
  function strike() {
    // 2-3 simultaneous main bolts shooting top→bottom
    const count = Math.floor(Math.random() * 2) + 2;
    for (let i = 0; i < count; i++) {
      const startX = W * (0.1 + Math.random() * 0.8);
      const endX   = startX + (Math.random() - 0.5) * W * 0.4;
      const segs   = generateBolt(startX, -20, clamp(endX, 10, W - 10), H + 20, 7, 0.75);
      activeBolts.push({ segs, life: 1, decay: 0.055 + Math.random() * 0.03 });

      // sparks along the bolt
      const sparksPerBolt = 18 + Math.floor(Math.random() * 12);
      segs.forEach(s => {
        if (Math.random() < sparksPerBolt / segs.length) {
          sparks.push(new Spark((s.x1 + s.x2) / 2, (s.y1 + s.y2) / 2));
        }
      });
    }

    // horizontal "edge crack" bolt across the middle of the screen
    const midY  = H * (0.3 + Math.random() * 0.4);
    const hSegs = generateBolt(0, midY, W, midY + (Math.random() - 0.5) * 60, 6, 0.65);
    activeBolts.push({ segs: hSegs, life: 1, decay: 0.07 });

    // screen flash
    flash.style.animation = 'none';
    flash.offsetHeight; // reflow
    flash.style.animation = 'lx-flash 0.45s ease-out forwards';

    // body shake
    document.body.style.animation = 'none';
    document.body.offsetHeight;
    document.body.style.animation = 'lx-shake 0.35s ease-out';
  ...
  const TRIGGER_ZONE   = 80;  // px from section boundary to fire
  const COOLDOWN_MS    = 900; // min ms between strikes
  ...
  function refreshBoundaries() {
    boundaries = Array.from(
      document.querySelectorAll('section, .marquee-section, footer')
    ).map(el => el.offsetTop).filter(y => y > 100);
  }
  ...
  window.addEventListener('scroll', () => {
    ...
    if (now - lastStrike < COOLDOWN_MS) { prevScrollY = scrollY; return; }

    for (const boundary of boundaries) {
      const prevDist = Math.abs(prevScrollY + H * 0.5 - boundary);
      const curDist  = Math.abs(scrollY  + H * 0.5 - boundary);
      // crossed the midpoint threshold
      if (prevDist > TRIGGER_ZONE && curDist <= TRIGGER_ZONE) {
        lastStrike  = now;
        strike();
        break;
      }
  ...
  /* expose for manual trigger (e.g., nav clicks) */
  window._lightning = strike;
})();
